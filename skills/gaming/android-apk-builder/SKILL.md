...
description: "为 Android 项目构建 APK 文件，支持本地构建、Termux 构建、在线构建（GitHub Actions）等多种方式。当用户要求构建 Android APK 时使用此技能。"
metadata:
---
...
- `scripts/build-apk.sh` - 本地/远程构建脚本
- `scripts/check-env.sh` - 环境检测脚本
- `scripts/termux-build.sh` - Termux 构建脚本

## 参考文档

- `references/周易算卦-构建指南.md` - 周易算卦项目构建指南
